...
def is_version_compatible(software_version: str, cpe_version: str) -> bool:
    try:
        sv = Version(software_version)
        cv = Version(cpe_version)
        return abs(sv.major - cv.major) <= 1 or str(sv.major) in cpe_version
...
def find_semantic_cpe_match(vendor: str, product: str, software_version: str, db: Session, threshold=0.8):
    build_cpe_index(db)
    ...
    cpe_vendor = matched_cpe.cpe_json.get("vendor", "").lower()
    cpe_product = matched_cpe.cpe_json.get("product", "").lower()
    cpe_version = matched_cpe.cpe_json.get("version", "")

    # Optional hard filter: full vendor match + token overlap + version compatibility
    if vendor.lower() != cpe_vendor:
        print(f"[EMBEDDING REJECTED] Vendor mismatch: '{vendor}' vs '{cpe_vendor}'", flush=True)
        return None

    tokens = set(clean_tokens(product))
    if not any(token in cpe_product for token in tokens):
        print(f"[EMBEDDING REJECTED] No product token match", flush=True)
        return None

    if not is_version_compatible(software_version, cpe_version):
        print(f"[EMBEDDING REJECTED] Version mismatch: {software_version} vs {cpe_version}", flush=True)
        return None
...
def find_best_cpe_match(db: Session, app_name: str, app_version: str, vendor: str = None):
    ...
    if no_version_matches:
        ...
        cpe_version = best_match.cpe_json.get("version", "")
        if not is_version_compatible(app_version, cpe_version):
            print(f"[NO-VERSION REJECTED] Version mismatch: {app_version} vs {cpe_version}", flush=True)
        else:
    ...
    semantic_cpe = find_semantic_cpe_match(vendor_normalized, app_name_normalized, app_version, db)
    if semantic_cpe:
        return semantic_cpe
