...
from app.core.database import BaseWorker as Base
...
class Agent(Base):
    ...
    software_links = relationship("AgentSoftwareInventory", back_populates="agent")
    os_update_links = relationship("AgentOSUpdateInventory", back_populates="agent")
    vulnerability = relationship("AgentVulnerabilityStatus", back_populates="agent")
...
class SoftwareInventory(Base):
    ...
    agent_links = relationship("AgentSoftwareInventory", back_populates="software", cascade="all, delete")
    cves = relationship("InventoryCVE", backref="software_inventory", cascade="all, delete")
...
class AgentSoftwareInventory(Base):
    ...
    action = Column(String(50), nullable=False)  # e.g., add, remove, upgraded_to
    status = Column(String(50), nullable=False)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())

    __table_args__ = (UniqueConstraint("agent_id", "software_inventory_id", name="uq_agent_software"),)

    agent = relationship("Agent", back_populates="software_links")
    software = relationship("SoftwareInventory", back_populates="agent_links")
...
class OSUpdatesInventory(Base):
    ...
    agent_links = relationship("AgentOSUpdateInventory", back_populates="os_update")
    organization = relationship("Organization")

class AgentOSUpdateInventory(Base):
    __tablename__ = "agent_os_update_inventory"

    id = Column(Integer, primary_key=True)
    agent_id = Column(Integer, ForeignKey("agents.id"), nullable=False)
    os_update_id = Column(Integer, ForeignKey("os_updates_inventory.id"), nullable=False)
    ...
    __table_args__ = (UniqueConstraint("agent_id", "os_update_id", name="uq_agent_os_update"),)

    agent = relationship("Agent")
    os_update = relationship("OSUpdatesInventory")
...
class InventoryCVE(Base):
    ...
    cve_group_id = Column(Integer, ForeignKey("cve_groups.id"), nullable=True) 
